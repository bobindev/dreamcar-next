...
const eventsData: EventData[] = [
    {
        eventTitle: 'Paradise City Theme Park',
        city: 'Incheon',
        description: 'Experience magic and wonder in Incheon with a visit to the night-themed indoor theme park Wonderbox at Paradise City!',
        imageSrc: '/img/events/INCHEON.webp',
    },
    {
        eventTitle: 'Taebaeksan Snow Festival',
...
const Events = () => {
    ...
    const filteredEvents: EventData[] = selectedCity 
        ? eventsData.filter((event) => event.city === selectedCity) 
        : eventsData;

    const firstEvent = filteredEvents.length > 0 ? filteredEvents[0] : null;
                ...
                <Stack className={'container'}>
                    <Stack className={'info-box'}>
                        <Box component={'div'} className={'left'}>
                            {firstEvent ? (
                                <>
                                    <h2>{firstEvent.eventTitle}</h2>
                                    <h3>{firstEvent.city}</h3>
                                    <Button variant="contained" color="primary">
                                        Learn More
                                    </Button>
                                </>
                        ...
                        {firstEvent && (
                            <Box className={'right'}>
                                <Image
                                    src={firstEvent.imageSrc}
                                    alt={firstEvent.eventTitle}
                                    width={800}
                                    height={500}
                                />
                            </Box>
