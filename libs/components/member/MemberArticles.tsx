...
const MemberArticles: NextPage = ({ initialInput, ...props }: any) => {
	...
	/** APOLLO REQUESTS **/
  const [likeTargetBoardArticle] = useMutation(CREATE_BOARD_ARTICLE)
  const {
  ...
  const likeArticleHandler = async (e: any, user: any, id: string) => {
    try {
      ...
      console.log('Error, likeArticleHandle', err.message);
	...
	if (device === 'mobile') {
		...
		return (
            ...
            likeArticleHanler={likeArticleHandler} key={boardArticle?._id} size={'small'} />;
					})}
				</Stack>
				{memberBoArticles?.length !== 0 && (
