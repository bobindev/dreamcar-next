...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const likeBoArticleHanler = async (user: any, id: any) => {
		try {
			...
			console.log('Error, likeBoArticleHanler:', err.message);
										...
										<Stack className="info">
											<Stack className="icon-info">
												{boardArticle?.meLiked && boardArticle?.meLiked[0]?.myFavorite ? (
													<ThumbUpAltIcon onClick={() => likeBoArticleHanler(user, boardArticle?._id)} />
												) : (
													<ThumbUpOffAltIcon  onClick={() => likeBoArticleHanler(user, boardArticle?._id)}/>
												)}
