...
const PropertyDetail: NextPage = ({ initialComment, ...props }: any) => {
  ...
  //     setProperty(getPropertyData.getProperty);
  //     setSlideImage(getPropertyData.getProperty.propertyImages[0]);
  //   }
  // }, [getPropertyData]);
	...
	const likePropertyHanler = async (user: T, id: string) => {
		try {
			if (!id) return;
			...
			console.log('Error, likePropertyHanler:', err.message);
    ...
    if (!formData.name || !formData.phone || !formData.email || !formData.message) {
      sweetErrorAlert('Please fill out all fields');
      return;
    }
      ...
      const result = await createMessage({
        variables: {
          input: {
            ...
            contactRefId: property?._id, // Assuming this is the correct reference ID
          },
        },
      });
			...
			<div id={'property-detail-page'}>
				<div className={'container'}>
						...
						<Stack className={'property-info-config'}>
								...
								<Stack className={'left-box'}>
									...
									<Stack className={'bottom-box'}>
										...
										<Stack className="option">
											...
											<Typography>{property?.propertyMileage} km</Typography>
										</Stack>
									</Stack>
								</Stack>
										...
										<Stack className="button-box">
											{property?.meLiked && property?.meLiked[0]?.myFavorite ? (
												...
												<FavoriteBorderIcon
													fontSize={'medium'}
													// @ts-ignore
													onClick={() => likePropertyHanler(user, property?._id)}
												/>
											)}
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									<img
										src={slideImage ? `${REACT_APP_API_URL}/${slideImage}` : '/img/property/car-static.webp'}
										alt={'main-image'}
									/>
								</Stack>
								...
								<Stack className={'options-config'}>
									...
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{property?.propertyMileage} km</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
									...
									<Stack className={'bottom'}>
										...
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												...
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Used Mileage</Typography>
													<Typography className={'data'}>{property?.propertyMileage} km</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Color</Typography>
                  ...
                  placeholder={'Hello, I am interested in \n' + '[Renovated property at  floor]'}></textarea>
								...
								<Stack className={'info-box'}>
									<Button className={'send-message'} onClick={handleSubmit}>
										...
										{destinationProperties.map((property: Property) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={property.propertyModel}>
													<PropertyBigCard
														property={property}
														likePropertyHanler={likePropertyHanler}
														key={property?._id}
													/>
												</SwiperSlide>
											);
